...
pub struct PoolState {
    ...
    /// Token A
    pub token0_vault: Address,
    /// Token B
    pub token1_vault: Address,
    /// Pool tokens are issued when A or B tokens are deposited.
    /// Pool tokens can be withdrawn back to the original A or B token.
    pub lp_mint: Address,
    /// Mint information for token A
    pub token0_mint: Address,
    /// Mint information for token B
    pub token1_mint: Address,
    /// token_0 program
    pub token0_program: Address,
    /// token_1 program
    pub token1_program: Address,
...
impl anchor_lang::Owner for PoolState {
    fn owner() -> anchor_lang::solana_program::pubkey::Pubkey {
        anchor_lang::solana_program::pubkey::Pubkey::from(crate::RAYDIUM_CP_SWAP_ID.to_bytes())
    }
}
